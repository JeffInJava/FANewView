...
public class ReadStationXmlUtil {
		...
		try{
			...
			for(Iterator i =root.elementIterator("Station");i.hasNext();) {   
				...
				if("1".equals(stationNumber)){
					...
					if(metroLineA.equals("16")||metroLineA.equals("17")||metroLineA.equals("41")){
						draw(init,185+angleStationA,"#259F3A",0,STATIONS.getString(metroLineA+stationName)+"u",x,y);
						draw(init,5+angleStationA,"#259F3A",0,STATIONS.getString(metroLineA+stationName)+"d",x,y);
					}else{
					...
					drawCircle(x,y,metroLineA,STATIONS.getString(metroLineA+stationName),stationName);
					writer.write("</g>\n");
					...
					drawCircle(a_x,a_y,metroLineA,STATIONS.getString(metroLineA+stationName),stationName);
					...
					drawCircle(b_x,b_y,metroLineB,STATIONS.getString(metroLineB+stationName),stationName);
	...
	public static void drawThreeStation(double p_angle,double angleStation,String station,boolean isSameSite,String stationId,String stationName,String metroLine){
	    ...
	    draw(init,185+angleStation,"#259F3A",0,stationId+"u",tp_x,tp_y);
		draw(init,5+angleStation,"#259F3A",0,stationId+"d",tp_x,tp_y);
		
		drawCircle(tp_x,tp_y,metroLine,stationId,stationName);
	}
	...
	public static void draw(double angle,double startFrom,String color,double alpha,String stationName,double x,double y){
        ...
        angleA = angleA * Math.PI / 180;
        startFrom = startFrom * Math.PI / 180;
        String line_id=stationName.substring(0,2);
        String tp_name=stationName;
        if(line_id.equals("06")||line_id.equals("07")||line_id.equals("08")||line_id.equals("13")||line_id.equals("17")||line_id.equals("41")){
        	if(stationName.indexOf("u")>0){
        		tp_name=stationName.replaceAll("u", "d");
        	}else{
        		tp_name=stationName.replaceAll("d","u");
        	}
        }
        String path="<path id=\"p"+tp_name+"\" d=\"M "+(x + r * Math.cos(startFrom))+","+(y + r * Math.sin(startFrom));
        for(int i= 1;i<= n; i++){
           startFrom = startFrom + angleA;
           angleMid = startFrom - angleA / 2;
	...
	public static void drawCircle(double x,double y,String lineId,String stationId,String stationName){
		try{
        	writer.write("<circle id=\"p"+stationId+"\" cx=\""+x+"\" cy=\""+y+"\" r=\"3\" stroke=\"#white\" stroke-width=\"1\" fill=\"white\" stationName=\""+lineId+"-"+stationName+"\"/>\n");
        	if(lineId.length()>1){
