 ...
 *
 */    
@JsonTagAnnotation(actionValue="/get_fluxperiods.action",namespace="/station",isJsonReturn=true)
public class GetFluxPeriod extends JsonTagTemplateDaoImpl implements IDoService {
	...
	public List findFluxPeriod() throws Exception{
		...
		if(f_date.compareTo(tempDate) > 0) {
			sql_tp1=" tbl_metro_fluxnew_"+start_date+" where ticket_type not in ('40','41','130','131','140','141') and substr(end_time,9,4) between "+startTime+ "and "+endTime;
			...
			sql_tp1=" tbl_metro_fluxnew_history partition(V"+start_date+"_FLUXNEW_HISTORY) where substr(end_time,9,4) between "+startTime+ "and "+endTime;
		}
		if(s_date.compareTo(tempDate) > 0) {
			sql_tp2=" tbl_metro_fluxnew_"+com_date+" where ticket_type not in ('40','41','130','131','140','141') and substr(end_time,9,4) between "+startTime+ "and "+endTime;
		}else{
			sql_tp2=" tbl_metro_fluxnew_history partition(V"+com_date+"_FLUXNEW_HISTORY) where substr(end_time,9,4) between "+startTime+ "and "+endTime;
		}
		...
		if(line_id==null||"00".equals(line_id)){
			...
			"( "+
			"  select time_period,sum(times) over(order by time_period) times from"+
			"  ("+
