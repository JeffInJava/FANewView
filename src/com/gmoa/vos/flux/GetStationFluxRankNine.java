...
/**
 * 根据日期、线路、进出换类型查询9号线三期车站客流排名
 * 日期格式：yyyymmdd
 *
 */    
@JsonTagAnnotation(actionValue="/get_ninestation_fluxrank.action",namespace="/station",isJsonReturn=true)
public class GetStationFluxRankNine extends JsonTagTemplateDaoImpl implements IDoService {
	...
	public Object doService() throws Exception {
		...
		sql="select t1.*,rownum rn from " +
			"("+
			"  select round(sum(aa.times)/100) times,bb.station_nm,'09' line_id from "+
			"  ( "+
			"     select station_id,sum("+total_times+") times from "+sql_tp+" group by station_id "+
			"  ) aa, "+
			"  (select station_id,trim(station_nm_cn) station_nm from viw_metro_station_name where ? between start_time and end_time and line_id='09' and station_id>'0943') bb "+
			"  where aa.station_id=bb.station_id group by bb.station_nm order by times desc" +
			") t1";
		list = jsonTagJdbcDao.getJdbcTemplate().queryForList(sql,start_date);
		return list;
	}
