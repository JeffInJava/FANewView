 ...
 *
 */
@JsonTagAnnotation(actionValue="/get_time_seg_flow.action",namespace="/jinbo",isJsonReturn=true)
public class GetTimesegFlow extends JsonTagTemplateDaoImpl implements IDoService{
	...
	public Object doService() throws Exception {
		...
		/*String stationLst = "select ot2.station_nm_cn as stationNmCn from "
				...
				+ " t1.station_nm_cn=t2.station_nm_cn) ot2 where ot1.station_id=ot2.station_id group by ot2.station_nm_cn order by ot2.station_nm_cn ";
		*/
		List<String> stationLst = new ArrayList<String>();
		stationLst.add("徐泾东");
		stationLst.add("诸光路");
		stationLst.add("虹桥火车站");
		
		List<StationChainCompare> lst = new ArrayList<StationChainCompare>();
		if(preDayData){
			String sql ="select bb.stationNmCn,bb.start_time as startTime,nvl(aa.times,0) as enterTimes,nvl(aa.exitTimes,0) exitTimes,nvl(aa.times,0) - nvl(bb.times,0) as enterDvalue,"
					+ " nvl(aa.exitTimes,0) - nvl(bb.exitTimes,0) as exitDvalue FROM(select b.stationNmCn,a.start_time,sum(a.times) times,sum(a.exitTimes) exitTimes "
					...
					+ " where aa.stationNmCn(+)=bb.stationNmCn and aa.start_time(+)=bb.start_time  order by decode(bb.stationNmCn,'徐泾东',1,'诸光路',2,'虹桥火车站',3),bb.start_time ";
		...
		if(weeFlag){
			...
			//大于1时，才会有数据
			if(hour>=1){
				String weeComp  ="select bb.stationNmCn,bb.start_time as startTime,nvl(aa.times,0) as enterTimes,nvl(aa.exitTimes,0) exitTimes,nvl(aa.times,0) - nvl(bb.times,0) as enterDvalue,"
						+ " nvl(aa.exitTimes,0) - nvl(bb.exitTimes,0) as exitDvalue FROM(select b.stationNmCn,a.start_time,sum(a.times) times,sum(a.exitTimes) exitTimes "
						+ " from (select station_id,substr(start_time, 9, 2) || ':' ||lpad(trunc(substr(start_time, 11, 2) / 30, 0) * 30,2,'0') start_time,"
