					...
					if(value.type === "folder") {
						...
						if('additionalParameters' in value
							&& 'item-selected' in value.additionalParameters 
								...
								$entity.find('i').not(".tree-item-name i").removeClass(self.options['unselected-icon']).addClass(self.options['selected-icon']);
		...
		selectItem: function (el) {
			...
			if (this.options.multiSelect) {
				...
				$all.removeClass('tree-selected')
					.find('i').not(".tree-item-name i").removeClass(this.options['selected-icon']).addClass(this.options['unselected-icon']);
				data.push($el.data());
			}

			if($el.hasClass('tree-selected')) {
				$el.removeClass('tree-selected');
				$el.find('i').not(".tree-item-name i").removeClass(this.options['selected-icon']).addClass(this.options['unselected-icon']);
			} else {
				$el.addClass ('tree-selected');
				$el.find('i').not(".tree-item-name i").removeClass(this.options['unselected-icon']).addClass(this.options['selected-icon']);
				if (this.options.multiSelect) {
					data.push( $el.data() );
				}
			}
