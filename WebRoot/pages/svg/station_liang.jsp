  ...
  <body style="height:100%;width:100%;background-color:#0B122E;">
      ...
      <script type="text/javascript">
		...
		$("#svgDiv").html("");
      		s = Snap("#svgDiv");
      		//加载svg文件
      		Snap.load("pages/svg/stations_liang.svg", function (f) {
			    svg = f.select("svg");
			    s.append(svg);
			    svg.select("g").drag();//设置拖拽
			    
			    svg.selectAll("circle").attr({fill:"#258CF9"});
			    
			    $.each(show_statons,function(i,v){
			    	if(i<20){
			    		Snap.select("#"+v).attr({fill:"#0EF1F2"});
			    	}else{
			    		Snap.select("#"+v).attr({fill:"#FFFFFF"});
			    	}
        		});
			    Snap.select("#人民广场").attr({fill:"#FFFFFF"});
			    Snap.select("#世纪大道").attr({fill:"#FFFFFF"});
		    });
