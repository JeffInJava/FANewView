...
<body>
	<div class="mainApp" id="cocc" style="width:auto;height:auto;">
		<div style="width:100%;" id="selBtnId" >
			<div id="paramsId" style="float:right;width:94%">
				<form id="form1">
					<table border="0">
						<tr>
						...
						<tr>
							<td>模&nbsp;&nbsp;板：<select name="model_id" id="model_id"></select>
							</td>
							<td>二级模板：<select name="addr_id" id="addr_id">
		...
		<div data-v-c5b8f8b4 data-v-0b39df2e class="point" id="zf">
			...
			<table id="coccTb" data-sort="sortDisabled" align="center" border="0" cellspacing="1" cellpadding="0" hidden>
				...
				<tbody v-for="station in stationLst">
					<tr>
						<td width="105" valign="middle" rowspan="2" colspan="1" style="word-break: break-all; border-width: 1px; border-style: solid;" align="center">{{station.stationNmCn}}<br />
						</td>
						<td width="105" valign="middle" style="word-break: break-all; border-width: 1px; border-style: solid;" align="center">出</td>
						<template v-for="cc in chainCompare"> <template v-if="station.stationNmCn == cc.stationNmCn">
						...
						<td width="105" valign="middle" align="center" style="border-width: 1px; border-style: solid;">
							<span v-if="parseInt(cc.exitDvalue)>0"><span style="color: #c35656;font-size:26px;font-weight:bold"><b>{{cc.exitDvalue}}</b></span></span>
					...
					<tr>
						...
						<td width="105" valign="middle" align="center" style="border-width: 1px; border-style: solid;">
							<span v-if="parseInt(cc.enterDvalue)>0"><span style="color: #c35656;font-size:26px;font-weight:bold"><b>{{cc.enterDvalue}}</b></span></span>
	    ...
	    function initParam(){
		 ...
		 new Vue({
			    ...
			    mounted: function () {
					...
					window.getVisitpfw=this.getVisitpfw;
				...
				methods : {
					...
					getVisitpfw : function() {
						...
						if(cocc_view == '0'){
							vueThis.slayer = true;
							if($('#model_id').val() && $('#addr_id').val()){
								vueThis.modelId=$('#model_id').val();
								vueThis.subModelName = $('#addr_id').val();
							}
						}
						...
						$.post("cocc/get_station_time_seg_flow.action", {
							"startTime" : startTime,
							"chainTime" : compareTime,
							"preview" : "0",
							"modelId" : vueThis.modelId,
							"subModelName" : vueThis.subModelName
						}, function(data) {
							vueThis.timeSeg = data.timeSeg;
							vueThis.stationLst = data.stationLst;
							vueThis.chainCompare = data.chainCompare;
							$("#coccTb").show();
						});
						if(parent.window.ftlvue.showStation){
							parent.window.getRefreshStationData(startTime,compareTime,vueThis.modelId,vueThis.subModelName);
						}
					}
				}
			})
