...
<body style="margin:0px;padding:0px;">
	...
	<div style="width: 100%;">
		<div id="mainStationZF" style="height: 600px;width: 100%;"></div>
	</div>
    ...
    <script type="text/javascript">
    	...
    	function loadStationComData(){
    		...
    		doPost("station/get_station_fluxcompare.action", {"start_date":start_date,"com_date":com_date,"sel_flag":"zf","flux_flag":tp}, function(data){
    			var tp_stations="";
    			var stations=[],add_pers=[],times=[],add_times=[],null_times=[];
	    		$.each(data,function(i,v){
	    			tp_stations+=v.STATION_NM+",";
	    			add_pers.push({value:v.ADD_PER,itemStyle:{normal:{color:'rgba(254,132,99,0.8)'}}});
	    			stations.push(v.STATION_NM);
	    		});
	    		stationZFOption.series[0].data=add_pers;
	    		stationZFOption.xAxis[0].data=stations;
	    		
	    		doPost("station/get_station_fluxcompare.action", {"start_date":start_date,"com_date":com_date,"sel_flag":"zf","flux_flag":tp,"stations":tp_stations}, function(datas){
		    	...
		    	//填充图表数据
			     ...
			     function (ec) {
			     	var chartStation = ec.init(document.getElementById('mainStationZF')); 
			     	chartStation.setOption(stationZFOption, true);
			     	
			     	ecConfig = require('echarts/config');
					chartStation.on(ecConfig.EVENT.CLICK, function (param) {
						if(param.seriesName=="对比增幅"){
    	...
    	stationZFOption = {
		    ...
		    tooltip : {
		        ...
		        formatter: function (p){
               	    return p[0].name+"<br/>增幅：+"+p[0].value+"%";
			    }
		    },
		    calculable : true,
		    grid : {'y':30,'y2':30,'x':80,'x2':60},
		    xAxis : [
		        {
		            type:'category',
